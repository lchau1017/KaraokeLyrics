...
class KyricsStateCalculator {
     ...
     */
    fun calculateState(lines: List<ISyncedLine>, currentTimeMs: Int, config: KyricsConfig): KyricsUiState {
        if (lines.isEmpty()) {
        ...
        val lineStates = lines.mapIndexed { index, line ->
            index to calculateLineState(
                line = line,
                lineIndex = index,
    ...
    fun calculateLineState(
        ...
        val blurRadius = calculateBlurRadius(
            isPlaying = isPlaying,
            hasPlayed = hasPlayed,
            distance = distanceFromCurrent,
            config = config
     ...
     */
    fun calculateScale(isPlaying: Boolean, config: KyricsConfig): Float = if (isPlaying && config.animation.enableLineAnimations) {
     ...
     */
    fun calculateBlurRadius(isPlaying: Boolean, hasPlayed: Boolean, distance: Int, config: KyricsConfig): Float {
        val effects = config.effects
